...
def draw_polygon(screen, depth_buffer, points, texture, light_val):
    ...
    dz_dx = ((z1 - z0)*(y2 - y0) - (z2 - z0)*(y1 - y0)) * inv_area
    dz_dy = ((z2 - z0)*(x1 - x0) - (z1 - z0)*(x2 - x0)) * inv_area
    ...
    z_row = z0 + (min_x - x0) * dz_dx + (min_y - y0) * dz_dy
    for y in range(min_y, max_y):
        ...
        for x in range(min_x, max_x):
            ...
            if e0_val >= 0 and e1_val >= 0 and e2_val >= 0:
                ...
                if (0 <= px < tex_w and 0 <= py < tex_h) and (z < depth_buffer[(x + y * width)]):
                    ...
                    pure_index = (x + y * width)
                    depth_buffer[pure_index] = z
                    idx = pure_index * 4
                    buffer[idx]     = b
                    buffer[idx + 1] = g
                    buffer[idx + 2] = r
