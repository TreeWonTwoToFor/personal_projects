...
def draw_polygon(screen, points, texture, light_val):
    ...
    for i in range(min_y, max_y):
        for j in range(min_x, max_x):
            tri_vals = list(inside_triangle(points[0], points[1], points[2], (j, i)))
            if not check_inside_triangle(tri_vals):
                continue
            u, v = 0, 0
            for k in range(3):
                area_mod = tri_vals[k]/tri_vals[-1]
                u += uv_list[k][0] * area_mod
                v += uv_list[k][1] * area_mod
            img_size = texture[0]
            ...
            buffer_offset = (j + i * ss[0]) * 4
            buffer.write(bytes([color[2], color[1], color[0], 0]), buffer_offset)

# this is how you calculate pixel values individually
#tu, tv = 0, 0
#for k in range(3):
#    area_mod = tri_vals[k]/tri_vals[-1]
#    tu += uv_list[k][0] * area_mod
#    tv += uv_list[k][1] * area_mod
