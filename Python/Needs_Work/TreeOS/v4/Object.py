...
class Object:
    ...
    def update_wireframe(self):
        ...
        self.collision_values = get_bounding_box_values_wireframe(self.model)
    ...
    def move_to_origin_wireframe(self):
        cp = self.center_point
        self.translate_wireframe(-cp[0], -cp[1], -cp[2])
        self.update_wireframe()
        
    def translate_wireframe(self, x, y, z):
        scalar_list = [x,y,z]
    ...
    def scale_wireframe(self, x, y, z):
        scalar_list = [x,y,z]
        for point_pair in self.model:
            for point in point_pair:
                for i in range(3):
    ...
    def rotate_wireframe(self, x, y, z):
        rotate_list = [x,y,z]
        for point_pair in self.model:
            for point in point_pair:
                for i in range(3):
...
def remove_reference(model):
    ...
    return new_model
    # below return assumed 3 sided polygons
    #return [
...
def get_bounding_box_values_wireframe(model):
    max_x, max_y, max_z = model[0][0][0], model[0][0][1], model[0][0][2]
    low_x, low_y, low_z = model[0][0][0], model[0][0][1], model[0][0][2]
    for point_pair in model:
        for point in point_pair:
