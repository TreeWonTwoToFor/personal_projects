...
def load_texture(file_name):
    try:
        ...
        img = Image.open("./Tools/RootEngine/Assets/Objects/missing_texture.bmp")
...
def draw_polygon(buffer, screen_size, depth_buffer, points, texture, light_val):
    ...
    # ensuring proper depth ordering by using inverses
    inv_z0 = 1.0 / z0
    inv_z1 = 1.0 / z1
    inv_z2 = 1.0 / z2
    u0 *= inv_z0
    ...
    if area == 0:
        return
    inv_area = 1.0 / area
    # created barycentric gradiants, and incorperated the invrese areas for depth scaling
    ...
    for y in range(min_y, max_y):
        ...
        for x in range(min_x, max_x):
            if e0_val >= 0 and e1_val >= 0 and e2_val >= 0:
                # corrects the uv values based on the depth
                z = 1.0 / inv_z
                u_corr = u * z
                v_corr = v * z
