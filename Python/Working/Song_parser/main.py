...
def get_tracks_spotify() -> list:
    track_list = []
    results = sp.current_user_playlists()
    for playlist in results["items"]:
        if playlist.get('id') == playlist_id:
            for num_of_hundreds in range(int(playlist_len/100)):
                for item in sp.playlist_tracks(playlist_id=playlist_id, offset=int(num_of_hundreds)*100)["items"]:
                    track_list.append(item)
    return track_list

def get_track_link(track_list: list) -> list:
    list = []
    for track in track_list:
        list.append(track.get("track").get("external_urls").get("spotify"))
    return list

def get_track_info(track_list: list) -> list:
    list = []
    for track in track_list:
...
def get_track_musixmatch(track_name: str, track_artist: str) -> str:
